...
A fundamental part of intelligent reasoning (\cite{lake2017building}, \cite{danks2014unifying}) is being able to make decisions under uncertain conditions. In some cases, a decision maker has enough information about his environment to make choices by calculating expected utilities. 

In other cases she may not have enough information to proceed in this way. One possible way is to try to \textit{learn} that missing information from the environment by interacting with it.

We consider the case where the uncertain environment is governed by a \textit{causal mechanism} unknown to the agent. 
...
Learning by interaction has been extensively studied in Reinforcement Learning \cite{sutton1998reinforcement}, but the most common used techniques are purely reactive (\cite{garnelo2016towards}) and do not consider any structure of the environment such as causal relations.

\section{Related Work}
One stage decision problems in which the decision maker will choose one action, and one action only, and then observe the consequences of it have been modelled as \textit{bandit problems}.

In a bandit problem (\cite{sutton1998reinforcement}) an agent chooses an \textit{arm} from a slot machine, observes a reward and then moves on to the next machine wich is of the same kind and whose initial settings are independent of the previous machine and action.
...
We attempt to model the interaction between the decision maker and the environment as a repeated game between two players: the original decision maker and a new abstract player called Nature. The base game will be the original decision problem.

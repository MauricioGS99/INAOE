...
\\
\indent Finalmente, otra ventaja que otorga el tener un modelo causal de un ambiente es que este conocimiento puede ser \textit{transferible} a problemas futuros que también se lleven a cabo en el mismo ambiente. \\
\\
Entonces, en conclusión, lo que se propone es utilizar la maqunaría de Teoría de Juegos para modelar una interacción entre un tomador de decisiones y un entorno incierto el cual es regido por un modelo causal. A través de las interacciones el tomador de decisiones aprenderá del modelo además de usarlo para conseguir su objetivo. En lo que sigue del documento, veremos con cierto detalle algunas de las herramientas teóricas que serán de utilidad para la realización de la investigación propuesta.
...
\indent \textit{...all that matters is that our concept appropriately tracks some relation(s) in the world that can ground and support the learning, reasoning and inferences that we do with our causal knowledge...}
		...
		Sea $(\Omega, \mathcal{F})$ un espacio medible, finito, y consideramos una relación binaria $\to  \subseteq \mathcal{F} \times \mathcal{F}$ que cumple:
		\begin{itemize}
...
Contenido tomado de \cite{shoham2008multiagent}
\subsubsection{Juegos extensivos con información impefecta}
Contenido tomado de \cite{shoham2008multiagent} y \cite{10.1007/978-94-010-0189-2_25}
\subsubsection{Juegos extensivos con información incompleta}
Contenido tomado de \cite{osborne1994course}
...
En un entorno incierto para el cual se cumplen una serie de relaciones causales ¿cómo puede un agente inteligente adquirir y utilizar conocimiento sobre estas relaciones a partir de su interacción con el entorno en un proceso de toma de decisiones? 
...
\\
\indent Un primer planteamiento sería considerar que las acciones de la naturaleza son completamente \textit{desinteresadas}; es decir, que la naturaleza no tiene ningún objetivo en particular. En \cite{eberhardt2008causal} el problema de descubrimiento causal es planteado también como un juego, en el cual la naturaleza escoge desde el inicio el modelo causal \textit{real} y el jugador debe escoger intervenciones a llevar a cabo con tal de descubrir el modelo causal. En el trabajo de este autor,  el objetivo es descubrir el modelo causal, no el aspecto de toma de decisiones.

\subsection{Hipótesis}
Bajo ciertas condiciones, un agente inteligente puede adquirir y utilizar conocimiento causal a partir de su interacción con un entorno incierto.

\section{Objetivos}
	\subsection{Objetivo General}
	La investigación que se propone llevar a cabo tiene dos objetivos generales; por un lado, encontrar la manera en la que un agente que se enfrenta a un entorno incierto pueda usar información causal de manera que ésta sea de ayuda al llevar a cabo una tarea; por otro lado, encontrar la manera en la que un agente adquiera información causal conforme interactúa con su entorno.
	\subsection{Objetivos específicos}
	Para llevar satisfacer los objetivos generales, hasta el momento se ha identificado que será necesario lo siguiente:
\begin{itemize}
\item Una forma de especificar creencias sobre estructura causal; esto es, encontrar una familia de distribuciones que representen de manera coherente creencias o conocimiento previo acerca de la estructura causal de un entorno. Para esto, existen dos casos a considerar; el primero, es suponer que el tomador de decisiones conoce el número máximo de variables que el modelo causal puede tener; en este caso, las creencias del agente pueden estar representadas por una distribución sobre un espacio de matrices de tamaño fijo, aprovechando la relación que existe entre grafos acíclicos dirigidos y matrices. Por otro lado, si el agente desconoce el número máximo de variables, entonces el grafo causal puede tener cualquier tamaño. En este caso, el camino a explorar es el considerar distribuciones de probabilidad sobre espacios de dimensión infinita, las cuales son conocidas como priors Bayesianos no paramétricos. 
\item Una manera de \textit{utilizar} las creencias causales que el agente tiene en un momento dado.
\item Un criterio de actualización de creencias causales que debe converger al verdadero modelo causal así como ser coherente con estudios de la psicología de la formación de creencias (\cite{larrouy2017mindreading}). Es importante mencionar que los juegos Bayesianos, en su definición estándar, proveen un mecanismo para la actualización general de creencias, pero se necesita una manera de actualizar creencias que haga uso de la estructura causal misma.
...
\item Un primer paso es modelar las acciones del agente y la respuesta del entorno se modelan como un juego de dos jugadores, uno de los cuales será el tomador de decisiones inicial, y el otro será un ente abstracto llamado Naturaleza que, suponemos, conoce el modelo causal y lleva a cabo sus jugadas a partir de lo dictado por éste. Los juegos sucederán de manera repetida para poder aprender de éste. 
\item En este momento, suponemos que la Naturaleza es indiferente a cualquier resultado y para esto se le asignan pagos iguales en cada resultado del juego. Esto puede modificarse en un futuro para modelar casos en los que la Naturaleza tenga ciertas \textit{intenciones}.
\item Como caso inicial, consideramos que el modelo causal es conocido en su totalidad para el tomador de decisiones. Para este caso, se encontrará un criterio de solución y se comparará con algoritmos del estado del arte de detección de acciones óptimas en un modelo causal.
\item En una siguiente etapa, se considera que el tomador de decisiones sólo conoce la \textit{estructura} del modelo causal. Para este caso, se encontrará una forma de aprender los valores de los parámetros del modelo así como una manera de usar esta información en futuras decisiones a tomar. Para este segundo caso, se debe construir una manera de expresar el conocimiento inicial acerca de la estructura que además de ser coherente con la manera en la que los seres humanos formamos creencias (\cite{larrouy2017mindreading}).
\item Para llevar a cabo el descubriminto del modelo, se define un número de juegos de observación, dentro de estos juegos el tomador de decisiones no modificará sus creencias causales, aunque éstas lo obliguen a tener un desempeño no óptimo.
\item Posteriormente, en el caso en el que el agente no conoce nada, sus creencias serán lo único con lo que pueda operar. Este conocimiento causal inicial, en el caso general, debe ser expresado en términos de probabilidades y para esto se construirá un \textit{prior} que consiste en una distribución sobre un espacio de matrices. Recordemos que una matriz (que cumple ciertas características) define un grafo, por lo que tener de esta manera estaremos especificando las probabilidades que un agente asigna, de manera subjetiva, a distintos modelos causales. Todavía no está determinado si consideraremos que el tomador de decisiones conoce el número máximo de variables que puede tener el modelo o no. De esto dependerá la \textit{clase} de distribución que se deba usar.
...
\item En el tercer caso, el agente no conocerá nada del modelo, pero tendrá \textit{creencias} sobre este; este caso generaliza los anteriores, pues en el primer y segundo caso las creencias con las que inicia el agente es la información que conoce sobre el modelo. En resumen, en los tres casos el agente contará con \textit{creencias} sobre el modelo causal verdadero; en el primer caso, sus creencias coincidirán plenamente con el modelo; en el segundo caso, sólo con la estructura y en el tercer caso, no tenemos garantía de nada. 
\end{itemize}
...
Consideremos ahora el caso en el cual el tomador de decisiones conoce la estructura del modelo, pero no conoce los valores de los parámetros de éste. Entonces, lo que el agente tendrá es una distribución de probabilidad sobre los posibles valores que estos parámetros puedan tener. 

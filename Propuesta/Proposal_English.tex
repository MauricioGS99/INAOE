...
Notice that the aforementioned papers consider learning while interacting with some (causal) environment. We propose to model this interaction as a game, which has been previously considered in \cite{werling2015job}, who model an \textit{on-line} classification problem as a game where an oracle is used to classify initial observations.
...
Consider a decision problem under uncertainty $(\mathcal{A},\mathcal{E},\mathcal{C},\succeq)$ where the available actions to a decision maker are causally related to outcomes and this causal relations are captured by a Causal Graphical Model $\mathcal{G}$. The Causal model is assumed to remain fixed and unknown to the decision maker, who is assumed to be rational and aware of the causal nature of his problem. 

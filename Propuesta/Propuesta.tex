...
Debido al enorme número de factores diversos que están involucrados en nuestro entorno, constántemente nos vemos forzados a tomar decisiones cuyas consecuencias son inciertas (\cite{danks2014unifying}). Esa misma realidad, altamente compleja e interconectada, contiene una serie de relaciones \textit{causa-efecto} que podemos utilizar al momento de tomar decisiones. El conocimiento causal permite preguntarnos  \textit{qué pasaría si...} (\cite{stalnaker2016knowledge}) e inferir las consecuencias de nuestros actos. Por desgracia, es difícil que contemos con la información necesaria para llevar a cabo estas predicciones de manera correcta, a lo más, podemos contar con estimaciones sobre lo probable que es la ocurrencia de algún evento incierto dada cierta acción.\\
\\
\indent En el lenguaje cotidiano, la palabra \textit{causa} evoca cierta regularidad o estabilidad del entorno; cuando hablamos de enfermedades existentes queremos pensar en las causas de esta para tomar acciones preventivas. Al estar aprendiendo a realizar cierta tarea prestamos atención a los efectos de alguna acción y asumimos que la acción causó el efecto y lo tendremos en mente para futuros escenarios similares.Tanto así, que los seres humanos formamos explicaciones o interpretaciones causales sin contar con toda la información disponible, lo cual puede llevar a errores sistemáticos al momento de tomar decisiones o emitir juicios (\cite{tversky1977causal}, \cite{tversky1980causal}, \cite{kahneman2011thinking}. Una pregunta que surge de manera natural es cómo utilizar esta información causal de manera efectiva para que un agente inteligente tome decisiones de manera correcta.\\
\\
\indent En este trabajo, consideramos un agente inteligente que debe tomar una decisión en un entorno incierto, el cual está \textit{gobernado} por un modelo causal fijo, pero desconocido para el agente. El agente sabe que sus acciones, el impacto de éstas en su entorno, y la consecuencia obtenida, están relacionadas de manera causal, por lo que buscará hacer uso de esta \textit{estructura} en su entorno para la toma de decisiones. \\
\\
\indent La Teoría de la Decisión bajo Incertidumbre, desarrollada en los trabajos de \cite{von1944theory}, \cite{definetti1930}, \cite{definetti1937}, \cite{savage1954the}, \cite{bernardo2000bayesian}, \cite{gilboa2009decision}, considera a un tomador de decisiones racional\footnote{Siguiendo a \cite{bernardo2000bayesian}, \cite{binmore2008rational},\cite{gilboa2009decision} decimos que un tomador de decisiones es racional si su relación de preferencias satisface una serie de axiomas.} que se enfrenta al problema de escoger una de entre varias opciones (o acciones) posibles en un escenario incierto. El tomador de decisiones expresa sus preferencias entre dos acciones en una relación binaria $\succeq$. Los trabajos de \cite{von1944theory}, \cite{definetti1937}, \cite{savage1954the} proveen de un criterio formal para la toma de decisiones, el cual consiste en la maximización de la utilidad esperada. La utilidad esperada, como su nombre lo indica es el valor esperado de la utilidad que le producen al tomador de decisiones las consecuencias de sus acciones; para poder calcularla, es necesario contar con estimaciones de la probabilidad de ocurrencia de ciertos eventos inciertos. \\
\\
\indent En diversas ocasiones, un tomador de decisiones no tiene a su alcance todos los parámetros (probabilidades de los eventos), pero puede intentar \textit{aprenderlos} del mismo entorno mediante la interacción con éste (\cite{sutton1998reinforcement}). Los métodos más comunes de Aprendizaje por Refuerzo aprenden políticas que son puramente reactivas; es decir, se construyen a partir de estimaciones estadísticas, pero no capturan ninguna estructura del entorno más allá de lo que se puede expresar en un Proceso de Decisión Markoviano (MDP por sus siglas en inglés).

\indent Debido a que las relaciones causa-efecto son un tipo de \textit{estructura} de un ambiente y que los seres humanos utilizamos y modificamos la información causal con la que contamos para escoger las acciones que lleven a cierto resultado deseado (\cite{sloman2006causal}, \cite{nichols2007decision}, \cite{meder2010observing}, \cite{hagmayer2013repeated}, \cite{danks2014unifying}), y aunque este aprendizaje no es perfecto  (\cite{rottman2014reasoning}), proponemos que un agente autónomo puede utilizar y adquirir información causal a partir de su interacción con su entorno, el cual es gobernado por un modelo causal. La manera en la que se propone que el agente tome en cuenta, aprenda y utilice la información causal del entorno es dotarlo de \textit{creencias} sobre el modelo causal del entorno, las cuales serán utilizadas y modificadas conforme el agente interactúa con el entorno.
\\
\indent Mientras que en Aprendizaje por Refuerzo la interacción agente-entorno se modela como transiciones entre estados así como una recompensa asociada a cada transición, en este trabajo se propone modelar la interacción entre el agente y el entorno como una \textit{juego}\footnote{Un juego es una situación estratégica en la que los intereses de dos o más actores se enfrentan y se afectan entre sí.} entre dos actores: el mismo agente o tomador de decisiones y un ente abstracto que llamaremos la Naturaleza, cuyas acciones estarán controladas por un modelo causal. Además, como el agente se enfrenta a problemas de decisión de una sola ronda, vamos a considerar \textit{juegos repetidos} en donde el juego a repetir (llamado \textit{juego base} en la literatura) está relacionado con el problema de decisión inicial. \\
\\
\indent En cada ronda del juego, el agente utilizará sus \textit{creencias} como si fueran un modelo causal verdadero, el cual utilizará para tomar una buena decisión. Posteriormente, a partir del resultado observado, las creencias serán modificadas de modo que se tome una mejor decisión en la siguiente ronda. De este modo, además de aprender una política, el agente estará aprendiendo un modelo causal de su entorno.\\
\\
...
\\
\indent Es deseable considerar el caso en el que un jugador no conoce todas las características del otro jugador; ya sea sus acciones o sus pagos, y es este desconocimiento lo que se pretende modelar con los Juegos Estratégicos Bayesianos.
...
\subsection{Justificación(aquí van como las aplicaciones)}
...
\node at ($(1)!.5!(2)$) {$M$};
\end{tikzpicture}
\caption{Forma extensiva del juego del médico vs la Naturaleza: la Naturaleza comienza asignando la enfermedad del paciente, y debido a que el médico no sabe cuál fue la jugada, debe decidir dentro de un conjunto de información, donde sus opciones son el fármaco, la cirugía o nada.}
\end{figure}
La evaluación en este caso se lleva a cabo comparando la utilidad acumulada del médico así como el número de rondas requeridas para llegar esta cantidad con el número de rondas que le toma a los algoritmos de identificación de mejor acción que no incorporan información causal \cite{audibert2010best}.
...
Por definir.
\subsection{Plan de publicaciones}
